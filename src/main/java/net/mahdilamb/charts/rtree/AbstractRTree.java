...
import static net.mahdilamb.charts.rtree.Node2D.distBBox;
import static net.mahdilamb.charts.rtree.Node2D.union;
import static net.mahdilamb.charts.rtree.RectangularNode.enlargedArea;
import static net.mahdilamb.charts.rtree.RectangularNode.intersectionArea;
...
abstract class AbstractRTree<T> {
     ...
     */
    public abstract Collection<Node2D<T>> getLeaves();
     ...
     */
    public abstract Collection<Node2D<T>> search(double minX, double minY, double maxX, double maxY);
    ...
    private List<Node2D<T>> searchRecursive0(double minX, double minY, double maxX, double maxY, Node2D<T> node, List<Node2D<T>> results) {
        if (node == null) {
            return results;
        }
    ...
    List<Node2D<T>> searchNonRecursive(double minX, double minY, double maxX, double maxY) {
    ...
    List<Node2D<T>> searchRecursive(double minX, double minY, double maxX, double maxY) {
    ...
    private List<Node2D<T>> getLeavesRecursive0(Node2D<T> node, List<Node2D<T>> result) {
        if (node == null) {
            return result;
        }
        if (node.leaf) {
     ...
     */
    protected List<Node2D<T>> getLeavesRecursive(Node2D<T> start, List<Node2D<T>> result) {
        return getLeavesRecursive0(start, result);
    }

    protected List<Node2D<T>> getLeavesNonRecursive(Node2D<T> node, List<Node2D<T>> result) {
     ...
     */
    public Collection<Node2D<T>> search(RectangularNode<T> range) {
