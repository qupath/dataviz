...
import static net.mahdilamb.charts.datasets.DatasetImpl.COLUMN_SEPARATOR;
 ...
 */
abstract class DataSeriesImpl<T> implements DataSeries<T> {
    ...
    public String toString() {
        ...
        if (size() > 1) {
            ...
            if (size() < MAX_VISIBLE_CELLS) {
                for (int i = 0; i < size(); ++i) {
                    DatasetImpl.formatCell(stringBuilder, String.valueOf(i), width).append(COLUMN_SEPARATOR).append(formatCell(this, i)).append('\n');
                ...
                //ellipses
                stringBuilder.append(StringUtils.repeatCharacter('.', Math.max(width, 1))).append(COLUMN_SEPARATOR).append("...\n");
        ...
        return stringBuilder.append(String.format("{Series: \"%s\", size: %d, type: %s}", getName(), size(), getType())).toString();
    }
